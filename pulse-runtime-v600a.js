...
(()=>{
  ...
  const runtime={
    targetVersion:'6.0',stage:'6.0A',currentVersion:'6.0A',
  ...
  window.render=function(){runtime.run(before);try{const view=runtime.getView(window.state?.tab);if(view?.body&&typeof window.layout==='function'){window.layout(view.body());const h=document.querySelector('.topbar h1');if(h&&view.title)h.textContent=typeof view.title==='function'?view.title():view.title;const s=document.querySelector('.page-subtitle');if(s&&view.subtitle)s.textContent=typeof view.subtitle==='function'?view.subtitle():view.subtitle}else baseRender.apply(this,arguments)}finally{runtime.run(after);runtime.paintVersion()}};
  const baseGo=window.go;if(typeof baseGo==='function')window.go=function(tab){const out=baseGo.apply(this,arguments);runtime.run(afterNav,tab);return out};
