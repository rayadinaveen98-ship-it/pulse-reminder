// Pulse V4.2.5 — authoritative cloud pull boundary
(()=>{
  const VERSION='4.2.5';
  ...
  const fingerprint=r=>JSON.stringify({title:r.title,notes:r.notes||'',category:r.category,type:r.type,priority:r.priority,due:r.due,repeat:r.repeat,alertOffsets:offsets(r),enabled:r.enabled!==false,persistent:!!r.persistent,subtasks:r.subtasks||[],completedSubtasks:r.completedSubtasks||[]});
  ...
  // Cloud pulls must NEVER call the cloud-wrapped save(). They only update the local cache.
  window.pulsePullAll=async function(){
    if(!pulseUser||pulsePulling||state?.modal)return;
    pulsePulling=true;
    try{
      const [rrq,ssq,aaq,hhq,usq]=await Promise.all([
        ...
        pulseCloud.from('reminder_alerts').select('*').eq('user_id',pulseUser.id),
      ...
      const err=rrq.error||ssq.error||aaq.error||hhq.error||usq.error;if(err)throw err;
      const subs=new Map();(ssq.data||[]).forEach(x=>{if(!subs.has(x.reminder_id))subs.set(x.reminder_id,[]);subs.get(x.reminder_id).push(x)});
      const alerts=new Map();(aaq.data||[]).filter(x=>x.status==='pending'&&(!x.alert_kind||x.alert_kind==='scheduled')).forEach(x=>{if(!alerts.has(x.reminder_id))alerts.set(x.reminder_id,[]);alerts.get(x.reminder_id).push(Number(x.offset_minutes||0))});
      reminders=(rrq.data||[]).map(x=>{const st=subs.get(x.id)||[],ao=[...new Set(alerts.get(x.id)||[0])].sort((a,b)=>b-a);return{id:x.source_local_id||x.id,title:x.title,category:x.category,type:x.reminder_type==='automation'?'smart':x.reminder_type,priority:x.priority,due:x.due_at,repeat:x.recurrence_label||'Never',alertOffsets:ao,alertOffset:ao[0]??0,notes:x.notes||'',enabled:x.status!=='paused',persistent:!!x.persistent,subtasks:st.map(y=>y.title),completedSubtasks:st.map((y,i)=>y.is_completed?i:null).filter(i=>i!==null),lastResult:null}});
  ...
  // One debounced realtime pull. A pull never feeds another push.
  window.pulseStartRealtime=async function(){
    if(!pulseUser)return;
    if(pulseRealtime)await pulseCloud.removeChannel(pulseRealtime);
    let timer;
    const refresh=()=>{clearTimeout(timer);timer=setTimeout(()=>{if(!pulseSyncing&&!pulsePulling&&!state?.modal)pulsePullAll().catch(pulseCloudError)},1200)};
  ...
  // Cancel any push timer accidentally queued by an older pull implementation before this file loaded.
